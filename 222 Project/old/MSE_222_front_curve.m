function Ball_data = MSE_222_front_curve(Ball_data, theta_1, theta_2, R )
    ...
    for int i= theta_start:delta_theta:theta_end
        ...
        New_data(index, 8) = xi + (R-r)*(1+cos(i)); %pos in x
        New_data(index, 9) = yi + (R-r)*sin(i); %pos in y
        ...
        New_data(index, 5) = sin(i)*(alpha*r)+(w^2)*r*cos(i);       %acceleration in x - check signs
        New_data(index, 6) = -cos(i)*(alpha*r)+(w^2)*r*sin(i);;        %acceleration in y
