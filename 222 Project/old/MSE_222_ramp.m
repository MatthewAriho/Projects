function Ball_data = MSE_222_ramp(Ball_data, length, angle)
    ...
    for i=delta_l:delta_l:angle
        %Use energy analysis to determine ang velocity from the initial, under
        %no slip condition
        index = ceil(i/delta_l);
        New_data(index, 1) = t+ index*delta_t; %time
        w=-sqrt(((I+m*r^2)*wi^2-2*m*g*i*sin(theta))/I); %angular velocity
        New_data(index, 4) = w;
        New_data(index, 2) = -w*r*cos(theta); %vel in x
        New_data(index, 3) = w*r*sin(theta);    %vel in y
        New_data(index, 8) = xi + i*cos(theta); %pos in x
        New_data(index, 9) = yi - i*sin(theta); %pos in y
       

        %Used force analysis with no friction to find ang acc and then
        %tangential and normal acceleration
        New_data(index, 5) = -alpha*r*cos(theta);       %acceleration in x
        New_data(index, 6) = alpha*r*sin(theta);        %acceleration in y
        New_data(index, 7) = alpha;
        New_data(index, 10) = m*New_data(index, 5)/sin(theta);   %magnitude of normal force acting on ball
